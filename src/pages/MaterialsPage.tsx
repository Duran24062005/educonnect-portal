...
import { calendarApi, type CalendarCatalog, type CalendarSession, type CalendarSessionInput } from '@/api/calendar';
...
const emptyCatalog: CalendarCatalog = { years: [], grades: [], groups: [], areas: [], teachers: [], aulas: [] };
const emptyForm: MaterialForm = { title: '', description: '', session_id: '', topic: '', resourceMode: 'file', link_url: '', file: null };
...
const MaterialsPage = () => {
  ...
  const catalogQuery = useQuery({
    queryKey: ['material-calendar-catalog'],
    queryFn: () => calendarApi.catalog('teacher'),
    enabled: isTeacher,
    staleTime: 60_000,
  });
  const sessions = sessionQuery.data?.sessions || [];
  const teacherReady = !sessionQuery.isLoading && !catalogQuery.isLoading && Boolean(catalogQuery.data);
  ...
  const openCreate = () => {
    ...
    setEditingMaterial(null);
    setForm({ ...emptyForm, session_id: selected?._id || '', topic: selected?.topic || '' });
    setFormOpen(true);
  };
  ...
  const handleSessionSave = async (input: CalendarSessionInput, sessionId?: string): Promise<CalendarSession> => {
    const saved = sessionId ? await calendarApi.update(sessionId, input) : await calendarApi.create(input);
    await queryClient.invalidateQueries({ queryKey: ['material-sessions'] });
    updateForm('session_id', saved.id);
    updateForm('topic', saved.topic);
    setSessionDialogOpen(false);
    toast.success(sessionId ? 'Sesión actualizada' : 'Sesión creada');
    return saved;
  };
  const handleSubmit = () => {
    if (!form.title.trim() || !form.session_id || !form.topic.trim()) {
      toast.error('Completa título, sesión y tema.');
      return;
    }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <Card className="border-border/70 shadow-sm">
          <CardContent className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <FilterSelect label="Sesión" value={sessionFilter} options={filterSessions.map((item) => ({ _id: item._id, name: `${dateLabel(item.start_at)} · ${item.topic || item.area.name}` }))} onChange={setSessionFilter} />
          </CardContent>
        </Card>
      ...
      {isTeacher && (
        <>
          <Dialog open={formOpen} onOpenChange={setFormOpen}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end"><Field label="Sesión"><Select value={form.session_id} onValueChange={(value) => { const next = sessions.find((item) => item._id === value); updateForm('session_id', value); updateForm('topic', next?.topic || ''); }}><SelectTrigger><SelectValue placeholder="Selecciona una sesión" /></SelectTrigger><SelectContent>{sessions.map((item) => <SelectItem key={item._id} value={item._id}>{dateLabel(item.start_at)} · {item.group.name} · {item.area.name}</SelectItem>)}</SelectContent></Select></Field><Button type="button" variant="outline" onClick={() => setSessionDialogOpen(true)}><Plus className="h-4 w-4" />Nueva sesión</Button></div>
                <Field label="Tema de la sesión" htmlFor="material-topic"><Textarea id="material-topic" value={form.topic} onChange={(event) => updateForm('topic', event.target.value)} rows={2} placeholder="Ej. Ecuaciones lineales" /></Field>
          ...
          {sessionDialogOpen && <CalendarSessionDialog open={sessionDialogOpen} onOpenChange={setSessionDialogOpen} session={null} role="teacher" catalog={catalogQuery.data || emptyCatalog} schoolYearId={catalogQuery.data?.years[0]?.id || ''} canEdit onSave={handleSessionSave} onCancelSession={async () => undefined} onActivateSession={async () => undefined} />}
        </>
      )}
    </DashboardLayout>
  );
